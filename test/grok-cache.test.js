...
test("migration session: store/lookup/evict transition from legacy to canonical in the same process", async () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "grok-mig-"));
  const legacyDir = path.join(root, "legacy");
  const canonicalDir = path.join(root, "canonical");
  fs.mkdirSync(legacyDir);
  fs.mkdirSync(canonicalDir);

  const legacyFile = path.join(legacyDir, "cache", "deliberation", "grok-files.json");
  const canonicalFile = path.join(canonicalDir, "deliberation", "grok-files.json");

  fs.mkdirSync(path.dirname(legacyFile), { recursive: true });
  // Write initial legacy cache file
  cache.writeCache(legacyFile, {
    version: 1,
    entries: {
      "K_LEGACY": { fileId: "file_L", size: 10, filename: "l.tf", uploadedAt: 1, expiresAt: Math.floor(Date.now() / 1000) + 3600, apiBase: "x", keyFp: "y" }
    }
  });

  // Temporarily modify env and reload cache module
  const oldEnv = { ...process.env };
  process.env.XDG_CACHE_HOME = canonicalDir;
  process.env.CLAUDE_CONFIG_DIR = legacyDir;

  const cachePath = require.resolve("../server/grok/cache.js");
  const pathsPath = require.resolve("../core/paths.js");
  delete require.cache[cachePath];
  delete require.cache[pathsPath];

  const migCache = require("../server/grok/cache.js");

  try {
    // Verify require-time paths are set correctly: CACHE_FILE points to legacy, CACHE_WRITE_FILE points to canonical
    assert.equal(migCache.CACHE_FILE, legacyFile);
    assert.equal(migCache.CACHE_WRITE_FILE, canonicalFile);
    assert.equal(fs.existsSync(canonicalFile), false);

    // 1. Lookup of legacy key on CACHE_FILE should succeed (reading legacy)
    const hit1 = migCache.lookup(migCache.CACHE_FILE, "K_LEGACY", { apiBase: "x", keyFp: "y" });
    assert.ok(hit1);
    assert.equal(hit1.fileId, "file_L");

    // 2. Store a new key onto CACHE_FILE. It should migrate: read legacy, merge new, write to canonical
    const entryNew = { fileId: "file_NEW", size: 20, filename: "n.tf", uploadedAt: 2, expiresAt: Math.floor(Date.now() / 1000) + 3600, apiBase: "x", keyFp: "y" };
    await migCache.store(migCache.CACHE_FILE, "K_NEW", entryNew);

    // Canonical file should now exist
    assert.equal(fs.existsSync(canonicalFile), true);

    // 3. Subsequent lookup of the new key in the same process should succeed (reading canonical)
    const hitNew = migCache.lookup(migCache.CACHE_FILE, "K_NEW", { apiBase: "x", keyFp: "y" });
    assert.ok(hitNew);
    assert.equal(hitNew.fileId, "file_NEW");

    // 4. Subsequent lookup of the legacy key in the same process should also succeed (reading canonical)
    const hitLegacyAfter = migCache.lookup(migCache.CACHE_FILE, "K_LEGACY", { apiBase: "x", keyFp: "y" });
    assert.ok(hitLegacyAfter);
    assert.equal(hitLegacyAfter.fileId, "file_L");

    // 5. Store a second new key. It should read from canonical (not legacy) and write to canonical, keeping both new and legacy
    const entrySecond = { fileId: "file_SECOND", size: 30, filename: "s.tf", uploadedAt: 3, expiresAt: Math.floor(Date.now() / 1000) + 3600, apiBase: "x", keyFp: "y" };
    await migCache.store(migCache.CACHE_FILE, "K_SECOND", entrySecond);

    const hitSecond = migCache.lookup(migCache.CACHE_FILE, "K_SECOND", { apiBase: "x", keyFp: "y" });
    assert.ok(hitSecond);
    assert.equal(hitSecond.fileId, "file_SECOND");

    const hitNewAfterSecond = migCache.lookup(migCache.CACHE_FILE, "K_NEW", { apiBase: "x", keyFp: "y" });
    assert.ok(hitNewAfterSecond);
    assert.equal(hitNewAfterSecond.fileId, "file_NEW");

  } finally {
    // Restore env and reload original cache module
    process.env = oldEnv;
    delete require.cache[cachePath];
    delete require.cache[pathsPath];
    require("../server/grok/cache.js"); // reload with clean defaults
