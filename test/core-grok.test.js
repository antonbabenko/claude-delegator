...
const fakeBridge = {
  ...
  runGrok: async ({ model }) => ({ text: `grok ${model}`, output: null }),       // real shape: { text, output }
  runWithFiles: async ({ model }) => ({ text: `grok+files ${model}`, output: null, refs: [], ownedIds: [] }),
  classifyGrokError: (status) => ({ errorKind: status === 401 ? "auth" : "unknown", retryable: false }),
  resolveReasoningEffort: (e) => e || "high",
};
...
test("GK3: ask WITH files routes through runWithFiles", async () => {
  ...
  assert.equal(r.text, "grok+files grok-4.3");
});
